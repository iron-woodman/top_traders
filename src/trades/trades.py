...
from src.messages.profit_trade_message import ProfitTradeMessage
import src.utils.logs as custom_logging
from src.utils.config_handler import telegram_config, settings_config
...
def is_trade_new(stored_trades, trade_api):
    ...
    if trade_api["updateTimeStamp"] < (int(time() * 1000) - 150000) and DEBUG == 'False':
        custom_logging.add_log(f"Trade {trade_api['symbol']} placed @ {trade_api['entryPrice']} is older than 2.5 minutes.")
        is_new = False
    if percentage_difference > 3 and DEBUG == 'False':
